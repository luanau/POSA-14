 ...
 * @brief This class tests the use of Java Threads and several implementations
...
public class SynchronizedQueue {
  ...
  public static class QueueAdapter<E> {
    ...
    public void put(E msg) throws InterruptedException, TimeoutException {
      ...
      mProducerCounter++;
      boolean timeoutValue =
          mQueue.offer(msg, TIMEOUT_SECONDS, TimeUnit.SECONDS);
   ...
   * @brief Enumerated type for return values of testing logic, has String for
   *        easy output.
   */
  public enum SynchronizedQueueResult {
    JOIN_NEVER_CALLED("Join() never called."), RAN_PROPERLY(
        "Threads Ran Properly."), TESTING_LOGIC_THREW_EXCEPTION(
        "Testing Logic threw Exception."), THREADS_NEVER_CREATED(
        "Threads never created."), THREADS_NEVER_INTERUPTED(
        "Threads never interrupted."),
    THREADS_NEVER_RAN("Threads never ran."), THREADS_THREW_EXCEPTION(
        "Thread threw an exception."), THREADS_TIMEDOUT(
    ...
    public void run() {
      for (int i = 0; i < mMaxIterations; i++)
        try {
          ...
          System.out.println("Thread properly interrupted by " + e.toString()
              + " in consumerRunnable");
  ...
  /**
   * This runnable loops for mMaxIterations and calls put() on mQueue to insert
   * the iteration number into the queue.
   */
  ...
  public static SynchronizedQueueResult testQueue(QueueAdapter<Integer> queue) {
    try {
      ...
      Thread consumer =
          new Thread(SynchronizedQueue.consumerRunnable, "consumer");
      Thread producer =
          new Thread(SynchronizedQueue.producerRunnable, "producer");
